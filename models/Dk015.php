...
/**
 * This is the model class for table "dkpp_015".
 *
 * @property string $id
 * @property string $pid
 ...
 */
class Dk015 extends \yii\db\ActiveRecord
{
    /**
     * @inheritdoc
     */
    ...
    public function helperLoadCodes($url)
    {
                ...
                if ($in[':children']) {
                    foreach ($in['children_array'] AS &$child) {
                        if (!isset($child[':id'])) {
                            ...
                            GetParentsCpv_REC($out, $child, $pid);
                        }
                    }
                }
            } else {
        ...
        $keys = array_map('strlen', array_keys($tmp3));
        array_multisort($keys, SORT_ASC, $tmp3);

        $keys = array_map('strlen', array_keys($tmp2));
        array_multisort($keys, SORT_ASC, $tmp2);
...
//        ksort($out);
    ...
    {
        $sql = "REPLACE INTO `dk015` (`id`, `pid`, `name`, `children`, `child_ids`)
