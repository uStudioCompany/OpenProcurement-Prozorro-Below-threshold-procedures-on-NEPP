...
function getExpandedFolder() {
    expanded = $(".tree").find(".expanded");
    cookiesData = [];
    expanded.each(function () {
        cookiesData.push(id = $(this)[0].id.split('-')[1]);
    });
    date = new Date();
    expire = date.getDate() + (365 * 24 * 60 * 60);
    setCookie('pages_tree', JSON.stringify(cookiesData), {
        expires: expire
    });
}
function MoveNode(item) {
    itemID = item[0].id.split('-')[1];
    console.log(itemID);
    if (parentID != -1) {
        getExpandedFolder();
        $.ajax({
            type: "POST",
            url: web_path + 'pages/pages-tree/move',
            cache: false,
            data: {'parentID' : parentID, 'itemID' : itemID},
            success: function (data) {
                $.pjax.reload({container:"#ptree"});
            },
            error: function (data) {
                $.pjax.reload({container:"#ptree"});
            }
